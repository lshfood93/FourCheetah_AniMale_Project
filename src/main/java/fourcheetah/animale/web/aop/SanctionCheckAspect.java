...
public class SanctionCheckAspect {
    ...
    // =========================================================
    // ✅ CHANGED: Referer 기반 이전 페이지 복귀 유틸
    // =========================================================
    private String resolveBackLocation(HttpServletRequest request) {
        String referer = request.getHeader("Referer");
        if (referer != null && !referer.trim().isEmpty()) {
            // 같은 서버의 URL인지 확인 (외부 리다이렉트 방지)
            String serverUrl = request.getScheme() + "://" + request.getServerName() + ":" + request.getServerPort();
            if (referer.startsWith(serverUrl)) {
                // context path 제거 후 반환
                return referer.substring(serverUrl.length());
            }
        }
        // Referer 없으면 메인페이지
        return "/mainPage";
    ...
    private Object handleBan(ProceedingJoinPoint joinPoint, HttpServletRequest request, HttpSession session) {
        ...
        // ✅ CHANGED: 일반 Controller - BAN은 로그인 페이지로 (로그아웃 처리이므로 고정)
    ...
    private Object handleSuspend(ProceedingJoinPoint joinPoint, HttpServletRequest request,
        ...
        if (isRestControllerMethod(methodName)) {
            Map<String, Object> body = new HashMap<>();
            body.put("fail", "정지 중인 계정은 이 기능을 사용할 수 없습니다.");
        ...
        request.setAttribute("location", resolveBackLocation(request));
        return "message";
    }
    ...
    private Object handleWarning(ProceedingJoinPoint joinPoint, HttpServletRequest request) {
        ...
        if (isRestControllerMethod(methodName)) {
            Map<String, Object> body = new HashMap<>();
            body.put("fail", "경고 상태로 이 기능을 사용할 수 없습니다.");
            return ResponseEntity.status(HttpStatus.FORBIDDEN).body(body);
        }
