...
public class ReplyDAO {
	...
	private static final String SELECT_REPLY_LIST_RECENT = "SELECT " 
			+ "  r.reply_id, " 
			+ "  r.board_id, "
			+ "  r.member_id, " 
	...
	private static final String SELECT_REPLY_LIST_OLDEST = "SELECT " 
			...
			+ "  CASE WHEN m.member_role = 'WITHDRAWN' THEN '탈퇴한 회원' "
			+ "       ELSE m.member_nickname END AS writer_nickname, "
			+ "  m.member_profile_image AS writer_profile_image, " // 프로필 이미지 추가
			+ "  r.reply_content, " + "  r.reply_created_at, " // 작성일 추가
			+ "  r.reply_updated_at, " // 수정일 추가
			+ "  m.member_profile_color  AS writer_profile_color, "
	...
	// =========================================================
	// UPDATE (본인 수정)
	private static final String UPDATE_REPLY = "UPDATE reply " 
			+ "SET reply_content = ? "
	...
	public ArrayList<ReplyDTO> selectAll(ReplyDTO dto) {
		...
		List<ReplyDTO> list = jdbcTemplate.query(sql, (rs, rowNum) -> {
			...
			data.setIsEdited(rs.getInt("is_edited"));  // isEdited 추가
			return data;
		}, dto.getBoardId());
	...
	public boolean insert(ReplyDTO dto) {
		...
		int rows = jdbcTemplate.update(INSERT_REPLY, 
				dto.getBoardId(), 
				dto.getMemberId(), 
				dto.getReplyContent());
	...
	public boolean update(ReplyDTO dto) {
		...
		if ("REPLY_UPDATE".equals(condition)) {
			// 본인 댓글 수정
			rows = jdbcTemplate.update(UPDATE_REPLY, 
					dto.getReplyContent(), 
					dto.getReplyId(), 
					dto.getMemberId());
		} else if ("REPLY_ADMIN_DELETE".equals(condition)) {
			// 관리자 삭제(내용 치환)
			rows = jdbcTemplate.update(UPDATE_REPLY_ADMIN_DELETE, 
					dto.getMemberId(), 
					dto.getReplyId());
		} else {
			return false;
	...
	public boolean delete(ReplyDTO dto) {
		...
		int rows = jdbcTemplate.update(DELETE_REPLY, 
				dto.getReplyId(), 
				dto.getMemberId());
