...
public class AdminReportServiceImpl implements AdminReportService {
    ...
    public boolean updateReportApprove(int boardId, int handledBy) {
        ...
        try {
            ...
            // ✅ CHANGED: 신고 사유 코드 저장 (이메일 발송에 사용)
            String reportReasonCode = reportDetail.getReasonCode();

            System.out.println("[1단계] 게시글 작성자 ID: " + boardWriterId);
            System.out.println("[1단계] 신고 사유 코드: " + reportReasonCode);
            ...
            if (newCount == 3) {
                ...
                // ✅ CHANGED: 제재 없는 경고 횟수도 WARNING으로 이메일 발송
                warningType = "WARNING";
                reason = "유효 신고 누적 " + newCount + "회 - 경고";
                System.out.println("[4단계] 제재 없음 - 경고 이메일 발송 (누적 " + newCount + "회)");
            }

            // ========================================
            // 【5단계】 이메일 발송
            // ========================================
            System.out.println("[5단계] 제재/경고 알림 이메일 발송 시작");

            try {
                // ✅ CHANGED: reportCount, reportReasonCode 파라미터 추가
                emailService.sendSanctionNotice(memberEmail, warningType, endAt, reason, newCount, reportReasonCode);
            ...
            System.out.println("  - 제재 타입: " + warningType);
