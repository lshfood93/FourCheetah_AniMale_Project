 ...
 * 
 * 통합 이전: - LoginController - LogoutController - JoinController -
 * ChangeProfileController - PasswordController - WithdrawController -
...
public class MemberController {
	...
	public MemberController(MemberService memberService, WithdrawRepository withdrawRepository) {
	...
	@PostMapping("/login")
	public String login(MemberDTO memberDTO, @RequestParam(value = "autoLogin", required = false) String autoLogin,
	...
	public String join(MemberDTO memberDTO, HttpServletRequest request) {
		...
		if (memberDTO == null || memberDTO.getMemberName() == null || memberDTO.getMemberName().trim().isEmpty()
		...
		System.out.println("[JOIN] verified=" + joinEmailVerified + ", verifiedEmail=" + verifiedEmail + ", formEmail="
				+ formEmail);
	...
	public String myPage(HttpSession session, Model model) {
		...
		// 매번 DB에서 새로 조회해서 memberData 채우기
		MemberDTO q = new MemberDTO();
		q.setCondition("MEMBER_MYPAGE");
		q.setMemberId(memberId);

		MemberDTO memberData = memberService.selectOne(q);
		if (memberData == null) {
			session.setAttribute("msg", "회원 정보를 불러오지 못했습니다.");
			return "redirect:/mainPage"; // 또는 mypage로 보내되 메시지 처리
		}
	...
	public String updateProfile(@RequestParam(required = false) String memberNickname,
			@RequestParam(required = false) String temporaryProfileImageToken,
			@RequestParam(required = false) String memberProfileColor,
			@RequestParam(required = false) String memberNicknameColor, HttpSession session, Model model) {
		...
		String newProfileColor = (memberProfileColor == null) ? "" : memberProfileColor.trim();
		String newNicknameColor = (memberNicknameColor == null) ? "" : memberNicknameColor.trim();
		...
		boolean nicknameColorChanged = (!newNicknameColor.isEmpty() && !newNicknameColor.equals(cur.getMemberNicknameColor()));
		 ...
		 if (profileColorRequested && !profileColorChanged
		         && !nickChanged && !profileChanged && !nicknameColorChanged) {
		     session.setAttribute("msg", "이미 적용된 테두리입니다");
		     return "redirect:/member/mypage";
		 }
		 if (nicknameColorRequested && !nicknameColorChanged
		         && !nickChanged && !profileChanged && !profileColorChanged) {
		     session.setAttribute("msg", "이미 적용된 색상입니다");
		     return "redirect:/member/mypage";
		 }
		
		String token = profileChanged ? temporaryProfileImageToken.trim() : null;

		if (!nickChanged && !profileChanged && !profileColorChanged && !nicknameColorChanged) {
		...
		if (!isAdmin) {
			...
			if (profileChanged)
				cost += 500;
			if (profileColorChanged)
				cost += 200;
			if (nicknameColorChanged)
				cost += 200;
		}
		...
		if (profileColorChanged)
			upd.setMemberProfileColor(newProfileColor);
		if (nicknameColorChanged)
			upd.setMemberNicknameColor(newNicknameColor);

		upd.setCondition(isAdmin ? "ADMIN_MEMBER_INFORM_UPDATE" : "MEMBER_INFORM_UPDATE");
		
		
		boolean ok = memberService.update(upd);
		if (!ok) {
			session.setAttribute("msg", isAdmin ? "수정 실패(DB 반영 실패)." : "수정 실패(캐시 부족 또는 DB 반영 실패).");
			return "redirect:/member/mypage";
		}
	...
	public String changePassword(@RequestParam(value = "currentPassword", required = false) String currentPassword,
			@RequestParam("newPassword") String newPassword, HttpSession session, HttpServletResponse response,
	...
	public String resetPassword(@RequestParam("memberPassword") String memberPassword, HttpSession session,
			RedirectAttributes ra) {
		Boolean verified = (Boolean) session.getAttribute("findPasswordVerified");
