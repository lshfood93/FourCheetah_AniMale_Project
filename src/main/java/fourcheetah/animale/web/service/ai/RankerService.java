...
public class RankerService {
    ...
    // =========================================================
    // "과부하 방지"용: bounded thread pool + bulkhead (semaphore)
    // - bounded queue: 큐가 꽉 차면 즉시 reject → 즉시 fallback
    // - bulkhead: 동시에 LLM 호출 가능한 개수 제한 (헬스 보호)
    // =========================================================
    private ThreadPoolExecutor executor;          //  Executors.newFixedThreadPool → bounded executor
    private Semaphore inFlightLimiter;            //  동시 호출 제한
    ...
    // future.get()의 대기 버퍼 (스케줄링/직렬화 오버헤드 약간 감안)
    ...
    private void initExecutor() {
        ...
        ThreadFactory tf = new ThreadFactory() { // 쓰레드 이름 부여 (로그 추적 편함)
        ...
        // AbortPolicy: 큐가 꽉 차면 RejectedExecutionException → 즉시 fallback
        ...
        this.executor.allowCoreThreadTimeOut(true); // 유휴 쓰레드 정리
    }

    // 종료 시 정리 (서버 종료/재시작 시 스레드 누수 방지)
    ...
    public List<RecommendedAnimeDTO> pickTopN(
        ...
        try {
            ...
            // =========================================================
            // 2) SDK 레벨 timeout을 request config에 부여
            ...
            // =========================================================
            // 3) bounded executor에 제출 + queueWait 로깅
            ...
            try {
                future = executor.submit(() -> {
                    ...
                    try {
                        ...
                        // inFlightLimiter는 "LLM 호출이 끝난 시점"에 반환
                        inFlightLimiter.release();
                    }
                });
            } catch (RejectedExecutionException rex) {
                // 큐 포화 → 즉시 fallback (여기서는 아직 작업이 실행되지 않았으므로 permit 반환)
                inFlightLimiter.release();
            ...
            // =========================================================
            // 4) hard timeout: timeoutMs + buffer
